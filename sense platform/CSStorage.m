...
- (void) databaseInit {
    ...
    //create the table
    const char *sql_stmt = "CREATE TABLE IF NOT EXISTS rawvalues (ID INTEGER PRIMARY KEY, timestamp real, sensor_name TEXT, sensor_description TEXT, device_type TEXT, device TEXT, data_type TEXT, value TEXT)";

    if (sqlite3_exec(db, sql_stmt, NULL, NULL, &errMsg) != SQLITE_OK) {
        pthread_mutex_unlock(&dbMutex);
        @throw [NSException exceptionWithName:@"DB error creating table rawvalues" reason:[NSString stringWithCString:errMsg encoding:NSUTF8StringEncoding] userInfo:nil];
    }

    //create table buffer
    const char *create_sql_stmt = "CREATE TABLE IF NOT EXISTS rawvalues (ID INTEGER PRIMARY KEY, timestamp real, sensor_name TEXT, sensor_description TEXT, device_type TEXT, device TEXT, data_type TEXT, value TEXT)";
    
    if (sqlite3_exec(db, create_sql_stmt, NULL, NULL, &errMsg) != SQLITE_OK) {
        pthread_mutex_unlock(&dbMutex);
        @throw [NSException exceptionWithName:@"DB error creating table buf.rawvalues" reason:[NSString stringWithCString:errMsg encoding:NSUTF8StringEncoding] userInfo:nil];
    }
    
    //create indexes
    //create index on timestamp as most data functions request data in a range
    const char *index5_stmt = "create index IF NOT EXISTS timestamp_index on rawvalues(timestamp)";
    if (sqlite3_exec(db, index5_stmt, NULL, NULL, &errMsg) != SQLITE_OK) {
        pthread_mutex_unlock(&dbMutex);
        @throw [NSException exceptionWithName:@"DB error creating index on rawvalues" reason:[NSString stringWithCString:errMsg encoding:NSUTF8StringEncoding] userInfo:nil];
    }

    //same for in memory data
    const char *index_stmt = "create index IF NOT EXISTS buf.timestamp_index on buf.rawvalues(timestamp)";
    if (sqlite3_exec(db, index_stmt, NULL, NULL, &errMsg) != SQLITE_OK) {
        pthread_mutex_unlock(&dbMutex);
        @throw [NSException exceptionWithName:@"DB error creating index on buf.rawvalues" reason:[NSString stringWithCString:errMsg encoding:NSUTF8StringEncoding] userInfo:nil];
    }
...
- (void) storeSensor:(NSString*) sensor description:(NSString*) description deviceType:(NSString*) deviceType device:(NSString*) device dataType:(NSString*) dataType value:(NSString*) value timestamp:(double) timestamp {
    //insert into db
    const char *sql_stmt = [[NSString stringWithFormat:@"INSERT INTO buf.rawvalues (id, timestamp, sensor_name, sensor_description, device_typem, device, data_type, value) VALUES (%lli, %f, '%@', '%@', '%@', '%@', '%@', '%@');", ++lastDataPointid , timestamp, sensor, description, deviceType, device, dataType, value] UTF8String];
...
- (NSArray*) getSensorDataPointsFromId:(long long) start limit:(size_t) limit {
    ...
    const char* query = [[NSString stringWithFormat:@"SELECT * FROM rawvalues where id >= %lli limit %zu", start, limit] UTF8String];
...
- (void) writeDbToFile {
    NSLog(@"Writing rawvalues buffer to file");
    
   const char* query = [[NSString stringWithFormat:@"insert into rawvalues (id, timestamp, sensor_name, sensor_description, device_type, device, data_type, value) select id, timestamp, sensor_name, sensor_description, device_type, device, data_type, value from buf.rawvalues as bv where id > %lli", lastRowIdInStorage] UTF8String];
    pthread_mutex_lock(&dbMutex);
    char* errMsg;
    BOOL succeed = YES;
    ...
    const char* queryRowId = "SELECT MAX(id) from rawvalues";
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, queryRowId, -1, &stmt, NULL) == SQLITE_OK) {
        NSInteger ret = sqlite3_step(stmt);
        if (ret == SQLITE_ROW) {
            lastRowIdInStorage = round(sqlite3_column_int64(stmt, 0));
        } else {
...
- (void) cleanBuffer {
    ...
    const char* query = [[NSString stringWithFormat:@"DELETE FROM buf.rawvalues where id <= %lli", lastRowIdInStorage] UTF8String];
    pthread_mutex_lock(&dbMutex);
...
- (void) cleanPersistentDb {
    ...
    const char* query = [[NSString stringWithFormat:@"DELETE FROM rawvalues where timestamp < %f", ([[NSDate date] timeIntervalSince1970] - 60.0 * 24 * 60 * 60)] UTF8String];
    pthread_mutex_lock(&dbMutex);
    if (sqlite3_exec(db, query, NULL, NULL, NULL) != SQLITE_OK) {
